...
class Http {
	...
	static public function getSupport() {
		...
		if (function_exists('curl_init')) {//curl方式
			...
			return 3;
		} else if (class_exists('SaeFetchurl')) {//sae
			return 4;
		} else {
			return 0;
		}
	...
	static public function doPost($url, $post_data=array(), $timeout=5, $header="") {
		...
		switch ($code) {
			...
			break;
			case 4:return self::saePoet($url, $timeout, $header);
			break;
			default:return false;
		}
	}
	...
	//通过sae post数据
	static public function saePost($url, $postdata=array(), $timeout=5, $header="") {
		...
		foreach ($files as $name => $file) {
			...
			$encoded .= join("", file(urldecode($file)));
			$encoded .= "\r\n";
		}
	...
	public function curlPostMethod($url, $method, $postfields = NULL) {
		echo "post";
		$ci = curl_init();
		curl_setopt($ci, CURLOPT_URL, $url);
		curl_setopt($ci, CURLOPT_CONNECTTIMEOUT, 10); // 连接超时
		curl_setopt($ci, CURLOPT_TIMEOUT, 10); // 执行超时
	...
	public function fsockopenPost($url, $post_data) {
		$srv_ip = 'www.teamcen.com';
		$srv_port = 80; //端口
		$fp = '';
		$errno = 0; //错误处理
		$errstr = ''; //错误处理
